...
import { AgentAvailability, AgentId, TaskStatus } from "../../../lib/domain/types";

type TaskDetail = {
  id: string; projectId: string; title: string; prompt: string; status: TaskStatus; agentId?: AgentId;
  run: { id: string; status: string; error: string | null; agent?: string; agentVersion?: string | null; agentAvailability?: AgentAvailability } | null;
...
function agentLabel(agent: string | undefined): string {
  ...
  if (agent === "cline") return "Cline";
  if (agent === "custom-cli") return "自定义 CLI";
  return agent ?? "Codex";
}
...
export default function TaskDetailClient({ taskId }: { taskId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    window.dispatchEvent(new Event("agentdeck:task-changed"));
  }, [taskId]);
  ...
  return <ConsoleShell projectId={detail.projectId} title={detail.title} meta={detail.worktree?.taskBranch ?? "当前工作区"}>
    ...
    <section className="task-command-header"><div><p className="eyebrow">任务详情</p><h1>{detail.title}</h1><p>{detail.prompt}</p></div><div className="task-state"><StatusBadge status={detail.status} /><span>{detail.worktree ? "隔离 Worktree" : "当前工作区"}</span><span>{detail.run?.agentVersion ? `${agentLabel(detail.run.agent ?? detail.agentId)} · ${detail.run.agentVersion}` : agentLabel(detail.run?.agent ?? detail.agentId)}</span></div></section>
    ...
    <div className="task-panels">
      <div className="task-primary-column"><HumanInputCard taskId={detail.id} requests={detail.humanInputRequests} onSubmitted={load} /><RunOutputPanel events={events} /><DiffViewer changedPaths={detail.review?.changedPaths ?? detail.changedPaths} diff={detail.review?.diff ?? detail.diff} baselineBranch={detail.worktree?.baselineBranch} baselineSha={detail.worktree?.baselineSha} taskBranch={detail.worktree?.taskBranch} commitSummary={detail.review?.commitSummary} /></div>
